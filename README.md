...
- 话题头节点标签显示真实加入状态（✅ / ➕），可直接加入；悬浮节点名称展示简介、公开状态与统计信息
...
- 通知窄栏在“赞”下方增加原生会员进度入口，按原生 `升级进度` 标签稳定定位，并等待异步菜单就绪后展示等级、账号状态与社区活跃度
- 钉钉顶栏显示器按钮改为“切换回 NodeLoc 原生界面”，原生模式下仍可通过右下角悬浮按钮返回 IM 界面
- 原生模式右下角恢复球独立常驻，在软路由、页面重建和首次加载后自动补挂，避免无法切回美化模式
- 创建话题支持慢网分阶段等待与三级降级重试；图标自动匹配 MIME 类型，并降低编辑器样式与原生弹窗监听的无效重复触发
