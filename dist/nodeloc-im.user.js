...
// @noframes
// @icon         https://www.nodeloc.com/favicon.ico
// @grant        none
...
(function() {
  "use strict";
  var _a, _b;
  const SKIN_KEY = "nodeloc-im-skin";
  const SKINS = {
    dingtalk: {
      label: "钉钉",
      orgName: "NodeLoc",
  ...
  function migratePrefs() {
    try {
      ...
      localStorage.setItem(SKIN_KEY, "dingtalk");
    } catch {
    }
  }
  ...
  function defaultOrgName() {
    return SKINS[SKIN_ID].orgName || "NodeLoc";
  }
  ...
  function otherThemeActive() {
    return !!document.getElementById("nodeloc-dingtalk-theme") || !!document.getElementById("nodeloc-feishu-theme") || document.documentElement.classList.contains("dingtalk-im-theme") || document.documentElement.classList.contains("feishu-im-theme");
  }
  const STYLE_ID = "nodeloc-im-theme";
  const FAVICON_ID = "im-favicon";
  const ROOT_CLASS$1 = "im-theme";
  const DARK_CLASS = "im-dark";
  const LOCK_CLASS = "im-locked";
  const VIEW_KEY = "nodeloc-im-view";
  const COLOR_THEME_KEY = "nodeloc-im-color-theme";
  const LAST_READ_KEY = "nodeloc-im-last-read";
  const LAST_READ_MAX_TOPICS = 200;
  const COMPOSE_PREVIEW_KEY = "nodeloc-im-compose-preview";
  const MASK_AVATAR_KEY = "nodeloc-im-mask-avatar";
  const MASK_TITLE_KEY = "nodeloc-im-mask-title";
  const HIDE_CAT_TAGS_KEY = "nodeloc-im-hide-cat-tags";
  const NAV2_KEY = "nodeloc-im-nav2";
  const ORG_NAME_KEY = "nodeloc-im-org-name";
  const ORG_ICON_KEY = "nodeloc-im-org-icon";
  const RAIL_W_KEY = "nodeloc-im-rail-width";
  ...
  const SPAM_FILTER_KEY = "nodeloc-im-spam-filter";
  const HIGHLIGHT_KEY = "nodeloc-im-highlight-keywords";
  const SEARCH_RECENT_KEY = "nodeloc-im-search-recent";
...
`;
  ...
  function isHomePath(pathname) {
    return pathname === "/" || /^\/(latest|new|unread|unseen|top|categories|hot|posted|read|bookmarks)\b/.test(pathname) || /^\/c\//.test(pathname) || /^\/n\/[^/]+\/?$/.test(pathname) || /^\/tag\//.test(pathname);
  }
  ...
  async function resolveListApiPath(apiPath) {
    const raw = String(apiPath || "");
    const match = raw.match(/^\/__nodeloc_node__\/([^?]+)(\?.*)?$/);
    if (!match) return raw;
    const slug = decodeURIComponent(match[1]);
    const node = await api(`/n/${encodeURIComponent(slug)}.json`);
    const category = node == null ? void 0 : node.category;
    if (!(category == null ? void 0 : category.id)) throw new Error("节点不存在或不可访问");
  ...
  async function loadCategories() {
    ...
    try {
      ...
      try {
        ...
        categoriesCache = ((_a2 = data.category_list) == null ? void 0 : _a2.categories) || [];
      } catch {
        categoriesCache = [];
      }
    }
  ...
  function isNodeCategory(category) {
    return !!(category == null ? void 0 : category.parent_category_id);
  }
  ...
  function renderRewards(post) {
    ...
    const people = rewards.slice(0, 8).map((r) => {
      const avatar = r.avatar_template ? `<img src="${escapeHtml(fullAvatarUrl(r.avatar_template))}" alt="" loading="lazy">` : `<span>${escapeHtml((r.username || "?").slice(0, 1).toUpperCase())}</span>`;
  ...
  function renderLottery(post) {
    ...
    const levels = (lottery.levels || []).map(
      (level) => `<li><strong>${escapeHtml(level.name || "奖项")}</strong><span>${escapeHtml(level.prize || "奖品")} × ${Number(level.quantity) || 1}</span></li>`
    ).join("");
    const winners = (lottery.winners || []).map(
      (winner) => `<a href="/u/${encodeURIComponent(winner.username || "")}">${escapeHtml(winner.username || "获奖者")} · ${escapeHtml(winner.prize || winner.level_name || "奖品")}</a>`
    ).join("");
  ...
  function renderRedEnvelope(post) {
    ...
    const state2 = envelope.status || envelope.state || "";
    return `<section class="im-plugin-card im-envelope-card">
    <div class="im-plugin-head"><strong>🧧 ${escapeHtml(label)}</strong><span>${escapeHtml(state2)}</span></div>
    <p>红包领取与余额操作保留在 NodeLoc 原生组件中。</p>
    ${nativeAction("打开原生红包")}
  </section>`;
  }
  function renderNodeLocPostExtras(post) {
...
[/quote]`)).catch(() => flashComposeHint("引用加载失败"));
  ...
  function closePostingGate() {
    var _a2;
    (_a2 = document.querySelector(".im-posting-gate")) == null ? void 0 : _a2.remove();
  }
  function showPostingGate(message, options = {}) {
    ...
    overlay.addEventListener("click", (event) => {
      if (event.target === overlay) closePostingGate();
    });
  ...
  function ensureRailResizer() {
    ...
    rz.addEventListener("pointerup", endDrag2);
    rz.addEventListener("pointercancel", endDrag2);
...
${item.label}`;
  ...
  function toggleChatWindow(drawer) {
    const root2 = document.documentElement;
    const windowed = !root2.classList.contains("im-chat-hub-windowed");
    root2.classList.toggle("im-chat-hub-windowed", windowed);
    if (windowed) {
      requestAnimationFrame(() => {
        const rect = drawer.getBoundingClientRect();
  ...
  function closeDingtalkChatHub({ closeNative = true } = {}) {
    ...
    (_c = drawer.querySelector('.c-navbar__toggle-drawer-button, button[title*="关闭聊天"], button[aria-label*="关闭聊天"]')) == null ? void 0 : _c.click();
  }
  ...
  function ensureRailDingtalk() {
    ...
    if (SKIN_ID === "wecom") {
      ...
      head.innerHTML = `<div class="im-rail-org-chip"><span class="im-rail-org-logo">NL</span><span class="im-rail-org-name">NodeLoc</span><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 4l3 3 3-3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></div>`;
    }
  ...
  function closeDingtalkProfileMenu({ closeNative = true } = {}) {
    ...
    root2.classList.remove("im-notif-open", "im-profile-open");
    menu == null ? void 0 : menu.classList.remove("im-user-menu-float", DINGTALK_PROFILE_CLASS);
    ...
    if ((toggle == null ? void 0 : toggle.getAttribute("aria-expanded")) === "true") {
  ...
  function buildItems(data) {
    ...
    for (const c of data.categories || []) {
      ...
      items.push({
        ...
        href: c.parent_category_id ? `/n/${encodeURIComponent(c.slug || c.id)}` : `/c/${encodeURIComponent(c.slug || c.id)}${c.id ? `/${c.id}` : ""}`
      });
    }
  ...
  function listSortSupported(path) {
    return path === "/" || path === "/latest" || path === "/categories" || /^\/c\//.test(path) || /^\/n\//.test(path) || /^\/tag\//.test(path);
  }
  ...
  async function loadList(apiPath, force) {
    ...
    if (!force && lastListFailPath === routeKey && Date.now() - lastListFailAt < 3e4) return;
  ...
  function toggleSkinMenu(anchor) {
    ...
    ).join("") + // 原版皮肤 = 退回 NodeLoc 原生界面（右下角悬浮球可切回 IM）
