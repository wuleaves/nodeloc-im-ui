...
// 书签复用原生 user-menu-bookmarks 接口，行渲染单独适配。
...
const TTL = 30_000;
const bookmarkBodySigs = new WeakMap();
...
/* ---------- 书签：/u/:username/user-menu-bookmarks（原生账户栏同源） ---------- */

function bookmarkItems(data) {
  if (Array.isArray(data?.bookmarks)) return data.bookmarks;
  if (Array.isArray(data?.user_bookmark_list?.bookmarks)) return data.user_bookmark_list.bookmarks;
  if (Array.isArray(data?.bookmark_list?.bookmarks)) return data.bookmark_list.bookmarks;
  return [];
}

function bookmarkMoreUrl(data) {
  return data?.more_bookmarks_url || data?.load_more_bookmarks ||
    data?.user_bookmark_list?.more_bookmarks_url || data?.bookmark_list?.more_bookmarks_url || null;
}

function bookmarkApiPath() {
  const username = getCurrentUsername();
  return username
    ? `/u/${encodeURIComponent(username)}/user-menu-bookmarks`
    : "/bookmarks.json";
}
...
function makeBookmarkSource() {
  ...
  async function load() {
    ...
    try {
      ...
      state.items = bookmarkItems(data);
      state.moreUrl = bookmarkMoreUrl(data);
      state.loadedAt = Date.now();
      state.error = null;
    } catch (err) {
      state.error = err?.message || "网络异常";
  ...
  function rowHtml(b) {
    const categoryId = b.category_id || b.topic?.category_id;
    ...
    const secondary = cat?.name || b.user?.username || (b.tags || []).join(" / ") || "书签";
    return `
  ...
  return {
    ...
    onScroll(body) {
      ...
      api(state.moreUrl)
        .then((data) => {
          const known = new Set(state.items.map((b) => b.id || b.bookmarkable_url).filter(Boolean));
          state.items = state.items.concat(bookmarkItems(data).filter((b) => !known.has(b.id || b.bookmarkable_url)));
          state.moreUrl = bookmarkMoreUrl(data);
          state.loadedAt = Date.now();
        })
        .catch(() => {})
